...
class TickDownloader:
    ...
    def initialize_mt5(self):
        ...
        print(
            f"Initialized {self.symbol}: Point={self.symbol_info.point}, Spread={self.symbol_info.spread}"
        )
        return True
    ...
    def download_ticks(self):
        ...
        ticks = mt5.copy_ticks_range(
            self.symbol, from_date, to_date, mt5.COPY_TICKS_ALL
        )

        if ticks is None or len(ticks) == 0:
            print("Failed to fetch ticks from range, trying alternate methods...")
            ticks = mt5.copy_ticks_from(
                self.symbol, from_date, 1000000, mt5.COPY_TICKS_ALL
            )
            if ticks is None or len(ticks) == 0:
                return []
...
class DAXTickDataLoader:
    ...
    def fetch_and_store_range(self, start_date, end_date):
        ...
        while current < end_date:
            month_end = (current + timedelta(days=32)).replace(day=1) - timedelta(
                seconds=1
            )
            ...
            path = (
                self.data_dir
                / self.symbol
                / f"{self.symbol}_{year}_{month:02d}.parquet"
            )
            ...
            if not os.path.exists(path):
                ...
                if today.month == month and today.year == year:
                    # Delete file and redo
                    os.remove(path)
                    print(
                        f"    File deleted (last modified: {today.strftime('%Y-%m-%d %H:%M:%S')})"
                    )
                    df = self.fetch_ticks_chunk(current, month_end)
                    if df is not None:
                        df.to_parquet(path, compression="zstd")
                    ...
                    print(
                        f"    File already exists (last modified: {today.strftime('%Y-%m-%d %H:%M:%S')})"
                    )
    ...
    def load_chunk(self, year, month):
        ...
        if not os.path.exists(path):  # path.exists():
            raise FileNotFoundError(
                f"The file at {path} cannot be found!. \nThe current directory is {os.getcwd()}"
            )
