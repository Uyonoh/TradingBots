...
class SignalPrecomputer:
    ...
    def get_daily_bias(df, buy_t, sell_t):
        mid = (df["bid"] + df["ask"]) / 2
        # Group by ny_tz Date
        days = mid.groupby(mid.index.tz_convert(ny_tz).date)
...
def get_todays_open_price(df, index, biases, config):
    ...
    # 3. Reconstruct Series with the provided ny_tz index
...
def process_chunk_parallel(year, month, config):
    ...
    df_ny_tz_idx = df.index.tz_convert(ny_tz)
    hours = df_ny_tz_idx.hour
    minutes = df_ny_tz_idx.minute
    daily_opens = get_todays_open_price(df, df_ny_tz_idx, biases, config)
...
#         results['server_entry_time'] = entry_time.dt.tz_convert(ny_tz)
...
The GER40 frequently experiences significant gaps between the previous day's close (5:30 PM ny_tz) and the main open the next morning (9:00 AM ny_tz).
...
4. The "Pivot Time" of 3:30 PM ny_tz
The U.S. markets (NYSE/Nasdaq) open at 3:30 PM ny_tz.
The Guarded Truth: This time often acts as a pivot point for the GER40. The index frequently pauses, reverses, or accelerates significantly at this exact time as a massive wave of U.S. volume hits the global markets. Many experienced traders avoid taking a new position immediately before 3:30 PM ny_tz, preferring to wait until after the initial U.S. open volatility subsides.
