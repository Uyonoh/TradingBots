...
class DataLoader:
    """
    Memory-optimized loading of MT5 candle data.
    Fetches data in monthly chunks, downcasts, saves as Parquet.
    """
    def __init__(self, symbol='GER40', data_dir='./candle_data', timeframe="M1"):
        self.symbol = symbol
        self.data_dir = Path(data_dir, symbol, timeframe)
    ...
    def fetch_chunk(self, start_dt, end_dt):
        ...
        data = mt5.copy_rates_range(self.symbol, timeframe, start_dt, end_dt)
        mt5.shutdown()
        ...
        df[['tick_volume','spread', 'real_volume']] = df[['tick_volume','spread', 'real_volume']].astype('int32')
        return df
    
    def fetch_and_store_range(self, start_date, end_date, ext="parquet"):
        ...
        while current < end_date:
            ...
            path = self.data_dir / f"{self.symbol}_{year}_{month:02d}.{ext}"

            if not os.path.exists(path):
                df = self.fetch_chunk(current, month_end)
                if df is not None:
                    if ext == "parquet":
...
if __name__ == "__main__":
    ...
    loader = DataLoader(symbol='GER40', data_dir='./tick_data', timeframe="M5")
