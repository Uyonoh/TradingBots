...
class SignalPrecomputer:
    ...
    def get_daily_bias(df, buy_t, sell_t):
        ...
        days = mid.groupby(mid.index.tz_convert(LONDON).date)
...
def get_todays_open_price(df, index, biases, config):
    ...
    # 3. Reconstruct Series with the provided LONDON index
...
def process_chunk_parallel(year, month, config):
    ...
    df_LONDON_idx = df.index.tz_convert(LONDON)
    hours = df_LONDON_idx.hour
    minutes = df_LONDON_idx.minute
    daily_opens = get_todays_open_price(df, df_LONDON_idx, biases, config)
...
if __name__ == "__main__":
    # Choose your path:
    choice = 0  # input("Enter 'B' for Backtest or 'O' for Optimize: ").upper()
    ...
    if choice == "O":
        ...
        if not results.empty:
            ...
            results["server_entry_time"] = entry_time.dt.tz_convert(LONDON)
...
The GER40 frequently experiences significant gaps between the previous day's close (5:30 PM LONDON) and the main open the next morning (9:00 AM LONDON).
...
4. The "Pivot Time" of 3:30 PM LONDON
The U.S. markets (NYSE/Nasdaq) open at 3:30 PM LONDON.
The Guarded Truth: This time often acts as a pivot point for the GER40. The index frequently pauses, reverses, or accelerates significantly at this exact time as a massive wave of U.S. volume hits the global markets. Many experienced traders avoid taking a new position immediately before 3:30 PM LONDON, preferring to wait until after the initial U.S. open volatility subsides.
