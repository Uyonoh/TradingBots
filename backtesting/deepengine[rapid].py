...
def process_chunk_parallel(year, month, config):
    ...
    n_shifts = 3
    price_shift = 2
    for _ in range(n_shifts + price_shift):
    ...
    for i in range(len(mids) - n_shifts - price_shift):
        curr_date = dates[i]
        ...
        shift_time = times[i + n_shifts + price_shift]
        shift_bid = bids[i + n_shifts + price_shift]
        shift_ask = asks[i + n_shifts + price_shift]
        ...
        if in_trade:
            ...
            if is_close_time or (i == len(mids) - 1):
                pnl = (shift_bid - entry_p) if direction == 'buy' else (entry_p - shift_ask)
            ...
            # if direction == "buy" and (n_ticks.sum() < sum_threshold/2):
            #     pnl = curr_bid - entry_p
            #     pnl *= contract_size # convert change in price to pips
            ...
            #     'direction': direction, 'profit_ticks': pnl, 'max_pnl': max_pnl, 'reason': 'tick_stop', 'current density': curr_den, 'avg density': avg_den}
            ...
            #     in_trade = False
            #     continue
            # elif direction == "sell" and (n_ticks.sum() > -sum_threshold/2):
            #     pnl = entry_p - curr_ask
            #     pnl *= contract_size # convert change in price to pips
            #     trade = {'date': curr_date, 'entry_time': entry_t, 'entry_price': entry_p, 'exit_time': curr_time, ' exit_price': curr_bid if direction == 'buy' else curr_ask,
            #     'direction': direction, 'profit_ticks': pnl, 'max_pnl': max_pnl, 'reason': 'tick_stop', 'current density': curr_den, 'avg density': avg_den}
