...
PRO_SETUP = {
    # GER40
    'bias_filter': {'buy_threshold': 0.6, 'sell_threshold': 0.4, 'buy_limit': 0.85, 'sell_limit': 0.15, 'bias_threshold': 1.5}, 
...
PRO_SETUP['risk_management']['trailing_stages'] = [
...
class SignalPrecomputer:
    ...
    def get_bias(df, buy_t, sell_t):
        ...
        p1 = df['close'] - df['close'].shift(1)
        p2 = df['close'].shift(1) - df['close'].shift(2)
        ...
        min_threshold = 20
        max_threshold = 200
        condition = (m0 == m1) & (p.abs() > min_threshold) & (p.abs() < max_threshold) & (p1.abs() < 100) & (p2.abs() < 100)
    ...
    def get_bias2(df, buy_t, sell_t):
        ...
        buy_mask = price_diff.between(min_threshold, max_threshold, inclusive='both')
        df.loc[buy_mask, '2m_bias'] = 'buy'

        # 3. Use .between() for the 'sell' logic
        sell_mask = price_diff.between(-max_threshold, -min_threshold, inclusive='both')
    ...
    @staticmethod
    def get_trend(df, buy_t, sell_t, lookback=5, threshold=50):
...
def process_chunk_parallel(year, month, config):
    ...
    # Handle Candle data
    # ===================================
    path = Path(f"./tick_data/{SYMBOL}/H1/{SYMBOL}_{year}_{month:02d}.csv")
    if not path.exists(): 
