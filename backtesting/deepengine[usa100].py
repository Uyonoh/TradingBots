...
# Value of 1 Lot moving 1 Tick (smallest move)
tick_val = info.trade_tick_value

# Smallest possible move in price
tick_size = info.trade_tick_size

# Value of 1 Lot moving 1.00 full Index Point / Pip
usd_per_lot_full_point = tick_val / tick_size
# ============================================
...
PRO_SETUP = {
    ...
    "risk_management": {
        ...
        "trailing_stages": [
            ...
            {
                "min_profit": np.int64(90),
                "max_profit": np.int64(120),
                "retention": 0.65,
            },
            {
        ...
        "tp_override": {
            "fast_threshold": np.int64(15),
            "slow_threshold": np.int64(120),
        },
    },
    "session_constraints": {
        "day_open": "09:00",
        "entry_start": "05:50",
        "mandatory_close": "23:30",
        "ghost_open": "08:00",
        "ghost_close": "08:15",
    },
...
PRO_SETUP = {
    "bias_filter": {"enabled": True, "buy_threshold": 0.6, "sell_threshold": 0.3},
    "entry_conditions": {
        "buffer_pips": np.int64(20),
        "velocity_multiplier": 1.9626786644578,
        "lookback_seconds": "3600S",
    },
    "risk_management": {
        "initial_sl_pips": [np.int64(50) * 0.75, np.int64(50)],
        "trailing_stages": [
            {"min_profit": 0, "max_profit": np.int64(30), "retention": -1},
            {"min_profit": np.int64(30), "max_profit": np.int64(60), "retention": 0.9},
            {"min_profit": np.int64(60), "max_profit": np.int64(90), "retention": 0.95},
            {
                "min_profit": np.int64(90),
                "max_profit": np.int64(120),
                "retention": 0.95,
...
PRO_SETUP = {
    ...
    "risk_management": {
        ...
        "trailing_stages": [
            {"min_profit": 0, "max_profit": np.int64(30), "retention": -1},
            {"min_profit": np.int64(30), "max_profit": np.int64(60), "retention": 0.5},
...
# PRO_SETUP = {'bias_filter': {'enabled': True, 'buy_threshold': 0.85, 'sell_threshold': 0.3}, 'entry_conditions': {'buffer_pips': np.int64(20), 'velocity_multiplier': 3, 'lookback_seconds': '2500S'}, 'risk_management': {'initial_sl_pips': [np.int64(50), np.int64(50)], 'trailing_stages': [{'min_profit': 0, 'max_profit': np.int64(10), 'retention': -1}, {'min_profit': np.int64(10), 'max_profit': np.int64(50), 'retention': 0.5}, {'min_profit': np.int64(50), 'max_profit': np.int64(90), 'retention': 0.7}, {'min_profit': np.int64(90), 'max_profit': np.int64(120), 'retention': 0.8}, {'min_profit': np.int64(120), 'max_profit': np.int64(150), 'retention': 0.9}, {'min_profit': np.int64(150), 'retention': 0.95}]}, 'session_constraints': {'day_open': '9:00', 'entry_start': '10:00', 'mandatory_close': '17:00', 'ghost_open': '8:00', 'ghost_close': '8:30'}} # GBPUSD
{
    ...
    "risk_management": {
        ...
        "trailing_stages": [
            {"min_profit": 0, "max_profit": np.int64(30), "retention": -1},
            {"min_profit": np.int64(30), "max_profit": np.int64(98), "retention": 0.76},
            {
                "min_profit": np.int64(98),
                "max_profit": np.int64(147),
                "retention": 0.81,
            },
            {
                "min_profit": np.int64(147),
                "max_profit": np.int64(196),
                "retention": 0.87,
            },
            {
                "min_profit": np.int64(196),
                "max_profit": np.int64(245),
                "retention": 0.92,
            },
            {"min_profit": np.int64(245), "retention": 0.95},
        ],
        "tp_override": {
            "fast_threshold": np.int64(15),
            "slow_threshold": np.int64(120),
...
PRO_SETUP = {
    "bias_filter": {
        "enabled": True,
        "buy_threshold": 0.6368957463292771,
        "sell_threshold": 0.32217086452287763,
    },
    "entry_conditions": {
        "buffer_pips": 6,
        "velocity_multiplier": 1.8869154175815386,
        "lookback_seconds": "3600S",
        "price_change_threshold": 50,
        "tick_loockback": 340,
    },
    "risk_management": {
        "initial_sl_pips": [100, 100],
        "trailing_stages": [
            {"min_profit": 0, "max_profit": 29, "retention": -1},
            {"min_profit": 29, "retention": 0.95},
        ],
    },
    "session_constraints": {
        "day_open": "16:30",
        "entry_start": "17:00",
        "mandatory_close": "22:30",
        "ghost_open": "16:07",
        "ghost_close": "16:15",
    },
}

PRO_SETUP = {'bias_filter': {'enabled': True, 'buy_threshold': 0.6635904557947964, 'sell_threshold': 0.3388939610722274}, 'entry_conditions': {'buffer_pips': 5, 'velocity_multiplier': 2.013254809911157, 'lookback_seconds': '3600S', 'price_change_threshold': 50, 'tick_loockback': 320}, 'risk_management': {'initial_sl_pips': [55, 55], 'trailing_stages': [{'min_profit': 0, 'max_profit': 46, 'retention': -1}, {'min_profit': 46, 'max_profit': 92, 'retention': 0.77}, {'min_profit': 92, 'retention': 0.95}]}, 'session_constraints': {'day_open': '15:30', 'entry_start': '16:28', 'mandatory_close': '21:58', 'ghost_open': '15:25', 'ghost_close': '15:34'}}
...
def process_chunk_parallel(year, month, config):
    ...
    for i in range(len(mids) - n_shifts):
        ...
        if daily_opens.get(curr_date, None) is None:
        ...
        if start < end:
            # Same-day session
            is_entry_window = start <= current < end
        else:
            # Overnight session
            is_entry_window = current >= start or current < end
        
        # Close condition (consistent with window definition)
        is_close_time = (
            (start < end and current >= end) or
            (start > end and current >= end and current < start)
        )
        ...
        if in_trade:
            ...
            multiplier = 1
            for s in stages:
                if max_pnl >= s["min_profit"] * multiplier:
                    if "max_profit" not in s or max_pnl <= s["max_profit"] * multiplier:
                        retention = s["retention"]

            if retention == -1:
                stop_level = (
                    (entry_p - (sl_initial * multiplier / contract_size))
                    if direction == "buy"
                    else (entry_p + (sl_initial * multiplier / contract_size))
                )
                if direction == "buy":
                    stop_level -= spread
                else:
        ...
        # ENTRY LOGIC
            ...
            n = config['entry_conditions']['tick_loockback']
            sum_threshold = config['entry_conditions']['price_change_threshold']
...
class DAXTickEngine:
    ...
    def run_backtest(self, start_year=2025, start_month=1, end_year=2025, end_month=12):
        ...
        use_cores = cores - 1
        print(
            f"Starting parallel engine for {SYMBOL} on {cores} cores (Limited to {use_cores} for RAM safety)..."
        )
...
if __name__ == "__main__":
    ...
    if choice == "O":
        ...
        if not results.empty:
            ...
            results["server_entry_time"] = entry_time.dt.tz_convert(CET)
            results.to_csv(f"{SYMBOL}_test.csv")
            win_rate = (results["profit_ticks"] > 0).mean() * 100
