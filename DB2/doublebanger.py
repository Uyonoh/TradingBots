...
class TradingBot:
    def __init__(self, symbol, log_level):
        symbol = symbol #.upper()
        self.symbol = symbol
        ...
        self.logger = trading_logger.setup_logging(symbol, log_level)
        ...
        try:
            ...
            self.logger.error(f"Error parsing symbol info: {e}")
            raise e
    ...
    def doublebanger(self, args, **kwargs):
        ...
        tp_pips = inputs["tp pips"] - (2 * self.spread)
...
def main(args_list=None):
    ...
    try:
        ...
        bot.logger.info(f"Unknown error: {e}")
