...
"""
First order against prev rc (wick?)
if profit:
    if price reversed toward rc (price > open || price < open) bor buy/sell:
        order along rc:
        if profit and price > close price:
            again
"""
...
def execute_trade(symbol, contract_size, direction, sl_pips, tp_pips=None):
    ...
    if not tp_pips is None:
        tp_points = tp_pips / contract_size
        tp_points += spread
...
#@retry(max_attempts=3, delay=1.0, exceptions=(OrderExecutionError,), logger=logger)
def modify_sl(symbol, ticket, new_sl, remove_tp=False, tp=None, contract_size=1):
    request = {
    ...
    if remove_tp and tp is not None:
        request["tp"] = tp + (10 / contract_size)
